...
function sort_by_ArrDelay() {
  ...
  ARR_DELAY=$(echo $FLIGHT_DATA | cut -d, -f15)

  cat << EOF
  
  O voo com maior atraso na chegada é o voo número: $FLIGHT_NUM
  Esse voo teve um atraso de $ARR_DELAY minutos na chegada.
